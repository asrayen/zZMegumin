...
bool isKey(char *s)
{ // 用来判断字符串是否为关键字,是则返回true,否则返回false
...
bool isOP(char *s)
{ // 用来判断字符串是否为运算符,是则返回true,否则返回false
...
int readToken()
{ // 用来根据空格从str中取词，并返回该词的长度，以便进行移位操作
    int i = 0;
...
LInfo STMT(LInfo info)
{
    if (flag)
    {
        ...
        { // 若是标识符
            ...
            if (num == -1)
            { // 因为是赋值语句，所以首先需要判断该标识符是否在前面已经声明过了，若未声明则报错
                string s = token;
                error = "标识符\"" + s + "\"未声明";
                flag = false;
                return info;
...
int main()
{
    ...
    {                  // 先进行词法分析
        ...
        if (str[location] == '#')
        {
            ...
            printSequence(fout);
            fout.close();
            cout << "四元式输出成功!请查收fourout.txt文件!" << endl;
        }
        else
        {
